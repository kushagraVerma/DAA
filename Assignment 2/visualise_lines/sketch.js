...
function takeInp(s){
    ...
    for(let ln of lnsplit){
        points = points.concat(ln[0]);
        let xs = ln[0].map(pt=>pt.x);
        let minX = Math.min(...xs);
        let maxX = Math.max(...xs);
        let y = (x)=>-(ln[1]*x+ln[2]);
        let line = {x1: minX, y1: y(minX), x2: maxX, y2: y(maxX)};
        lines.push(line);
        lineColors.push(color(random(100,255),random(100,255),random(100,255)));
    }
...
function draw() {
    ...
    for(let i = 0; i < lines.length; i++){
        ...
        stroke(col);
        line(ln.x1*ds,ln.y1*ds,ln.x2*ds,ln.y2*ds);
    }
    pop();
}
